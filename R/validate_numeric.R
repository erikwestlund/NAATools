...
#'   - invalid_rows: Data frame of rows with non-numeric values
#'   - invalid_catalog: Catalog of invalid values and their row numbers
#'   - invalid_numeric_count: Number of non-numeric values
#'   - is_valid: Boolean indicating if all values are numeric
#'   - message: Description of validation result
#'   - status: "error", "warn", or "success"
#' @export
validate_numeric <- function(duckdb_conn, table_name = "data", var, params = NULL) {
  # Get total row count
  total_rows <- get_duckdb_row_count(duckdb_conn, table_name)
  
  # Build validation query - check for non-numeric values (excluding NULL and empty)
  query <- sprintf("
    WITH numeric_check AS (
  ...
  counts_fn <- function(result) {
    list(
      invalid_numeric_count = nrow(result)
    )
  }
  
  message_fn <- function(result, counts, var) {
    if (nrow(result) == 0) {
      sprintf("All non-empty values in column '%s' are valid numeric values", var)
    } else {
      sprintf("Column '%s' contains %d non-numeric values", 
              var, counts$invalid_numeric_count)
    }
  }
